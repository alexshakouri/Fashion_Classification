...
NUM_CLASSES = 228
NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = 50000 #1st 50,000 of fasion train
NUM_EXAMPLES_PER_EPOCH_FOR_EVAL = 10000  #next 1000 of fasion train
...
def _read_py_function(filename, label):
  ...
  image_decoded=tf.image.decode_jpeg(filename.decode(),channels=3)
...
print('Train data set type: ',train_dataset.output_types)
print('Train dataset shape: ',train_dataset.output_shapes)
print('Eval data set type: ',eval_dataset.output_types)
print('Eval dataset shape: ',eval_dataset.output_shapes)
'''
...
# how many images 
    ...
    train_dataset = train_dataset.map(
        lambda filename, label: tuple(tf.py_func(
            _read_py_function, [filename, label], [tf.uint8, label.dtype])))
    train_dataset = train_dataset.map(_resize_function)
    train_dataset_iter = tf.data.Iterator.from_structure(train_dataset.output_types,train_dataset.output_shapes)
...
# how many images 
    ...
    eval_dataset = eval_dataset.map(
        lambda filename, label: tuple(tf.py_func(
            _read_py_function, [filename, label], [tf.uint8, label.dtype])))
    eval_dataset = eval_dataset.map(_resize_function)
    eval_dataset_iter = tf.data.Iterator.from_structure(eval_dataset.output_types,
                                           eval_dataset.output_shapes)
    
    return eval_dataset_iter
