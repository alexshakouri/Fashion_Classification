...
def writeSub(Y_directory):
    ...
    for i in range(0, y_Test.shape[0]):
        label_indices = np.array(np.where(y_Test[i,:]>0))+1
    ...
    submission.to_csv("D:\PrivacyPreservingDistributedDL\Filters\Submissions\submission{}.csv".format(datetime.datetime.now().strftime("%y-%m-%d-%H-%M")), index=False)